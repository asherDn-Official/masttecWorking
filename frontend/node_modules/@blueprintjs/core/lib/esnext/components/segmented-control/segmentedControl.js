...
export const SegmentedControl = React.forwardRef((props, ref) => {
    const { className, defaultValue, fill, inline, intent, large, onValueChange, options, role = "radiogroup", small, value: controlledValue, ...htmlProps } = props;
    ...
    return (React.createElement("div", { ...removeNonHTMLProps(htmlProps), role: role, onKeyDown: handleKeyDown, className: classes, ref: mergeRefs(ref, outerRef) }, options.map((option, index) => {
        ...
        return (React.createElement(SegmentedControlOption, { ...option, intent: intent, isSelected: isSelected, key: option.value, large: large, onClick: handleOptionClick, small: small, ...(role === "radiogroup"
                ? {
...
function SegmentedControlOption({ isSelected, label, onClick, value, ...buttonProps }) {
    ...
    return React.createElement(Button, { ...buttonProps, onClick: handleClick, minimal: !isSelected, text: label });
}
